...
function AddFunctionModel(props: ViewComponentProps) {
  const request = {};
  const methods = {
    refresh() {
      ...
      return _content.map((arg) => {
        return {
          ...
          functions: arg.functions.map((v) => {
            return {
              text: `add(1, "${v}")`,
              v: ui.$clock.methods.add(1, v).format("YYYY-MM-DD HH:mm:ss"),
            };
          }),
        };
      });
...
export const AddFunctionView = (props: ViewComponentProps) => {
  const [state, vm] = useViewModel(AddFunctionModel, [props]);

  return (
    <>
      <PageView store={vm}>
        <div class="text-3xl text-w-fg-0">add</div>
